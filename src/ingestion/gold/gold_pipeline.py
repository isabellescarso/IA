...
from ingestion.gold_transformations import GoldTransformationPipeline
...
class GoldIngestionPipeline:
    def __init__(self, reader: SilverParquetReader, writer: GoldParquetWriter):
        self._reader = reader
        self._writer = writer

    def run(self, patient_id: str) -> dict:
        raw = self._reader.read(patient_id)
        transformed = GoldTransformationPipeline().transform(raw)
